...
class wb_switch(SwitchEntity):
    def __init__(self, device: WBMr, cannel: int):
        ...
        self._attr_name = f"Реле {cannel+1}"
        self._device = device
        #self._attr_unique_id = self._attr_name
        self._attr_unique_id = f"{device.name}_switch_{cannel+1}"
        self._cannel = cannel
        self._attr_is_on = self._device.get_switch_status(self._cannel)
        #self._attr_entity_category = EntityCategory.CONFIG  # DIAGNOSTIC

    async def async_turn_off(self, **kwargs):
        """Turn the entity off."""
        self._attr_is_on = False
        await self._device.set_switch_status(self._cannel, self._attr_is_on)
